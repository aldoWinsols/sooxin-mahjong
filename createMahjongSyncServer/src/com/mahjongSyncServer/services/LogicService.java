...
public class LogicService extends Observable {
	...
	public void dealTimer(){
		if(!roomService.getRoom().isBeginGame()){
			return;
		}
		if(logicSwitch){
...
//			mahjongValue = playerService.getAction().checkDingzhangIsOver();
	...
	public void dingzhang(int azimuth, int mahjongValue){
		...
		System.out.println(playerService.getPlayer().getPlayerName() + " 玩家定章： " + azimuth);
//		if(playerService.getPlayer().getPlayerType() == 1){
//			operationPlayerService = playerService;
//			logicSwitch = true;
//			timeNum = 0;
...
//		deleteObservers();
		timeNum = 0;					//时钟计时数据
		logicSwitch = false;			//逻辑启动开关
		operationName = "";		//要执行的操作名字
		str = "";
		operationTimeNum = 0;		//操作需要的时间
		operationPlayerService = null;
		operationMahjongValue = 0;
		isHandleOperation = true;					//是否处理操作
		handleTime = 0;
		handleTimeNum = 0;
		onOffline = false;							//执行的是否是掉线还是在线 
		isFirstGetmahjongOver = false;				//是否是第一次摸牌完毕
		androidTimerNum = 0;							//执行机器人接入方法的时间
