...
//			}
	 ...
	 * @param roomNum
	 */
	public void createRoomService(String playerName, String roomNo, int enterNumber, 
			String roomPassword, int maxFanNum, int roomNum){
		...
		if(playerService == null){
			log.error("用户名不存在!");
			return;
		}else if(getRoomServiceByRoomNo(roomNo) != null){
			log.error("房间已经创建了!");
			return;
		}
		
		RoomService roomService = new RoomService();
		roomService.getRoom().setRoomNo(roomNo);
		roomService.getRoom().setEnterNumber(enterNumber);
		roomService.getRoom().setRoomPassword(roomPassword);
		roomService.getRoom().setMaxFanNum(maxFanNum);
		roomService.getRoom().setRoomNum(roomNum);
		
		gameHall.getRoomServices().add(roomService);
		
		roomService.addPlayerServices(playerService);
		
		log.info(playerName + " 玩家创建一个房间，房间名字：" + roomNo + " , 密码：" + roomPassword);
	 ...
	 * @param roomNo
	 */
	public void enterRoom(String playerName, String roomNo, String password){
		PlayerService playerService = getPlayerServiceByPlayerName(playerName);
		RoomService roomService = getRoomServiceByRoomNo(roomNo);
		if(playerService == null){
			log.error("用户名不存在!");
			return;
			...
			log.error("此房间不存在!");
			return;
		}else if(!roomService.getRoom().getRoomPassword().equals(password)){
			log.error("输入的密码不匹配!");
			return;
		}else if(roomService.getRoom().getPlayerServices().size() >= 4){
			log.error("房间人数已满!");
			return;
		}
		...
		log.info(playerName + " 玩家进入游戏：" + roomNo);
	}
...
//						j --;
//					}else{
						PlayerService playerService = gameHall.getRoomServices().get(i).getRoom().getPlayerServices().get(j);
//						j --;
						playerService.getPlayer().setOnlineState(1);
						setWhereFallline(playerService.getPlayer().getPlayerName(), 0);
//						remoteService.getUserService().exitGame(playerService.getPlayer().getPlayerName(), getOnlineStatus());
