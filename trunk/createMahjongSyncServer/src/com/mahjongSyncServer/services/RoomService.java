...
//		clearAllData();
		boolean isOnlineState = true;
		for (PlayerService playerService : room.getPlayerServices()) {
			if(playerService.getPlayer().getOnlineState() == 1){
				isOnlineState = false;
			}
		}
		// 有四个人的时候就开始游戏
		if (room.getPlayerServices().size() == 4 
				&& !room.isBeginGame() && isOnlineState) {
			return true;
		}
		return false;
	}
	...
	public void beginGame() {
		room.getBalanceService().baseNum = room.getRoomNum();
		room.getBalanceService().maxFunnum = room.getMaxFanNum();
	...
	public void addPlayerServices(PlayerService playerService) {
		...
		if(room.getPlayerServices().size() == 4){
			...
			room.setPlayerServices(getRandowWaitplayersPaixu(room.getPlayerServices()));
			
			int length = room.getPlayerServices().size();
			for (int i = 0; i < length; i++) {
				room.getPlayerServices().get(i).getPlayer().setOnlineState(2);
				room.getPlayerServices().get(i).getPlayer().setAzimuth(i + 1);
			}
		}
	}
	/**
